...
async def edit_application_choice(message: types.Message, state: FSMContext):
    ...
    Показуємо 2 кнопки: "Відкрити форму редагування" і "Скасувати".
    """
    kb = types.ReplyKeyboardMarkup(resize_keyboard=True, one_time_keyboard=True)
    kb.add("Відкрити форму редагування", "Скасувати")
    await message.answer("Відкрийте онлайн-форму для редагування:", reply_markup=kb)
    await ApplicationStates.editing_choice.set()


@dp.message_handler(Text(equals="Скасувати"), state=ApplicationStates.editing_choice)
async def cancel_editing_choice(message: types.Message, state: FSMContext):
    """
    Повертаємось до детального перегляду заявки, як ніби користувач тільки-но її відкрив.
    """
    data = await state.get_data()
    idx = data.get("selected_app_index")
    if idx is None:
...
async def open_form_for_editing(message: types.Message, state: FSMContext):
    ...
    index = data.get("selected_app_index")
    if index is None:
        await message.answer("Немає даних для заявки.", reply_markup=remove_keyboard())
        await state.finish()
        return
    ...
    kb = types.ReplyKeyboardMarkup(resize_keyboard=True, one_time_keyboard=True)
    kb.add(types.KeyboardButton("Редагувати заявку в WebApp", web_app=types.WebAppInfo(url=url_with_data)))
    kb.row("Скасувати")

    await message.answer("Відкрийте скорочену форму WebApp для редагування:", reply_markup=kb)
...
############################################
# Робота з WebApp (звичайне створення заявки)
############################################
